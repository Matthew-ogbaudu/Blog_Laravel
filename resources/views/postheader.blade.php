<header class="max-w-xl mx-auto mt-20 text-center">
    ...
    isset($currentcategory)?$currentcategory->name:'Categories'}}</button>
            ...
            <a href="/" class="block text-left px-3 text-sm
                leading-6 hover:bg-blue-300 focus:bg-blue-300" hover:text-white focus:text-white>
                All</a>

            @foreach($categories as $category)
                <a href="/categories/{{$category->slug}}"
                   class="block text-left px-3 text-sm leading-6 hover:bg-blue-300 focus:bg-blue-300"
                   hover:text-white focus:text-white > {{ucwords($category->name)}}</a>
            @endforeach
